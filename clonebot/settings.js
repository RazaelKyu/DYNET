...
msg.chat = dy.decodeJid(msg.key.remoteJid || message.message?.senderKeyDistributionMessage?.groupId || '')
msg.now = msg.messageTimestamp
msg.isGroup = msg.chat.endsWith('@g.us')
msg.sender = dy.decodeJid(msg.key.fromMe && dy.user.id || msg.participant || msg.key.participant || msg.chat || '')
}
if (msg.message) {
...
msg.msg.key.fromMe = dy.decodeJid(msg.msg.key.participant) === dy.decodeJid(dy.user.id)
if (!msg.msg.key.fromMe && msg.msg.key.remoteJid === dy.decodeJid(dy.user.id)) msg.msg.key.remoteJid = msg.sender
}
msg.text = msg.msg || ''
...
msg.quoted.chat = dy.decodeJid(msg.msg.contextInfo.remoteJid || msg.chat || msg.sender)
msg.quoted.isBaileys = msg.quoted.id && msg.quoted.id.length === 16 || false
msg.quoted.sender = dy.decodeJid(msg.msg.contextInfo.participant)
...
if (msg.quoted.url || msg.quoted.directPath) msg.quoted.download = () => dy.downloadMediaMessage(msg.quoted)
msg.quoted.copy = () => smsg(dy, M.fromObject(M.toObject(vM)))
}
}
if (msg.msg && msg.msg.url) msg.download = (saveToFile = false) => dy.downloadM(msg.msg, msg.mtype.replace(/message/i, ''), saveToFile)
return msg
}
