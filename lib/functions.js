...
ments = (teks) => {return teks.match('@') ? [...teks.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net') : [m.sender]}

pickRandom = async (list) => {
...
switcher = (status, isTrue, isFalse) => {
      return (status) ? texted('bold', isTrue) : texted('bold', isFalse)
   }
