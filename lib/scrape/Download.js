...
async function getRandomUserAgent() {
  const fakeUserAgent = new FakeUserAgent()
  return {
...
async function instagramdl(url) {
	return new Promise(async (res, rej) => {
		if (!url.match(/\/(reel|reels|p|stories|tv|s)\/[a-zA-Z0-9_-]+/i)) {
			return ect({ status: false, creator: "Nzx", msg: "Invalid Url" });
		}
		...
		try {
			...
			let olve = { status: true, data, }
			res(olve);
      
		} catch (e) {
			let ect = { status: false, msg: new Error(e).message }
			rej(ect);
		}
	});
}
...
async function getInfo() {
  try {
    ...
    const formInputs = document.querySelectorAll('form#get_video input[name]')
    
    return Array.from(formInputs).reduce((acc, input) => {
...
async function scrapeTwitterMedia(url) {
  ...
  try {
    ...
    const { window } = new JSDOM(json.data)
    const document = window.document
    const mediaElements = document.querySelectorAll('.videotikmate')
    
    return Promise.all(Array.from(mediaElements).map(async (element) => ({
      linkMedia: element.querySelector('img, video, audio')?.getAttribute('src'),
      username: element.querySelector('h1 a')?.textContent
    })))
  } catch (error) {
    throw new Error(`Twitter media: ${error.message}`)
  }
}
...
async function aio_dl(e) {
	try {
		let a = new URLSearchParams();
		a.append("url", e);
		a.append("token", "58deb9e8d4a27bc9bc59ff84619aa9421ed67801d07514c2fc8f4e63cf38e8ee");
		
		let response = await axios.post("https://www.aiodownloader.in/wp-json/aio-dl/video-data/", a, {
			headers: {
				authority: "www.aiodownloader.in",
				accept: "*/*",
				"accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
				"content-type": "application/x-www-form-urlencoded",
				cookie: "PHPSESSID=c022070d82b4cafadf3d776711afb539; pll_language=en",
				origin: "https://www.aiodownloader.in",
				referer: "https://www.aiodownloader.in/",
		...
		let t = o.medias ? o.medias.filter(e => e.extension === "mp4") : [];
		return {
			info: i,
			media: t
		};
...
/**
zeemo downloader YouTube 
@Tio
**/

async function zeemo(url) {
    try {
